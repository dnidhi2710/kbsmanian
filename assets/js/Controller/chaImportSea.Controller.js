...
app.controller('chaImportSeaController', ['$scope', '$location', '$window', '$http', 'usSpinnerService', 'dbConfigService',
	function ($scope, $location, $window, http, spinner, dbConfigService) {
		...
		$scope.navigate = function (jobid) {
			jobid = encodeSlash(jobid);
			$location.path("/createImportSea/" + jobid);
		}
