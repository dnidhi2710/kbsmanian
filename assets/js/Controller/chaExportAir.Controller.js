app.controller('chaExportAirController', ['$scope', '$location', '$window', '$http', 'usSpinnerService', 'dbConfigService', '$routeParams',
	function ($scope, $location, $window, http, spinner, dbConfigService, $routeParams) {
		/* Config */

		$scope.typeName = $routeParams.type;
		$scope.exportType = "exportAir";
		...
		$scope.navigate = function (id, type) {
			id = encodeSlash(id);
			$location.path("/createExport/" + id + "/" + type);
		}
